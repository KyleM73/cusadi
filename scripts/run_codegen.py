...
def main(args):
    casadi_fns = []
    fn_dir = cu.CUSADI_BENCHMARK_DIR if args.codegen_benchmark_fns else cu.CUSADI_FUNCTION_DIR
    for filename in os.listdir(fn_dir):
        ...
        if os.path.isfile(f) and f.endswith(".casadi"):
            if args.fn_name == "all" or args.fn_name in f:
                ...
                casadi_fns.append(ca.Function.load(f))
    for f in casadi_fns:
        if args.precision == "double":
            ...
            cu.generateCUDACodeFloat(f)
        else:
            raise ValueError(f"Invalid value for --precision={args.precision}, must be double or float")
        if args.gen_pytorch:
            cu.generatePytorchCode(f)
    cu.generateCMakeLists(casadi_fns, dtype_str=args.precision)
    t_compile = time.time()
    compileCUDACode()
